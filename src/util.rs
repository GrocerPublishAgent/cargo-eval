...
mod suppress_child_output {
    ...
    */
    pub fn suppress_child_output(cmd: &mut Command, timeout: Duration) -> Result<ChildToken> {
        ...
        let (done_sig, done_gate) = std::sync::mpsc::channel::<bool>();

        let _ = thread::spawn(move || {
            let show_stderr;
            let mut recv_done = false;
            match done_gate.recv_timeout(timeout) {
                Ok(success) => {
                    show_stderr = !success;
                    recv_done = true;
                },
                Err(cause) => {
                    debug_assert!(cause == std::sync::mpsc::RecvTimeoutError::Timeout);
                    show_stderr = true;
                }
            }
    ...
    pub struct ChildToken {
        child: process::Child,
        done_sig: Option<std::sync::mpsc::Sender<bool>>,
